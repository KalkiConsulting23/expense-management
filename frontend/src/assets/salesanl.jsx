...
function RevenueChart({ data }) {
  ...
  useEffect(() => {
    ...
    const series = Array.from(byMonth, ([k, v]) => ({ month: k, revenue: v }))
    ...
    g.selectAll('circle').data(series).join('circle')
      ...
      .on('mouseover', (event, d) => {
        tooltip.style('opacity', 1).html(`<b>${d.month}</b><br/>${fmt(d.revenue)}`)
          .style('left', `${event.offsetX + 12}px`).style('top', `${event.offsetY - 28}px`)
      })
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      <div id="rev-tooltip" style={{
        position: 'absolute', background: CARD, border: `1px solid ${ACCENT}`,
        borderRadius: 6, padding: '6px 10px', fontSize: 12, color: TEXT,
...
function SalesBarChart({ data }) {
  ...
  useEffect(() => {
    ...
    const series = Array.from(byMonth, ([k, v]) => ({ month: k, count: v }))
      .sort((a, b) => a.month.localeCompare(b.month))
    ...
    g.selectAll('rect').data(series).join('rect')
      .attr('x', d => x(d.month)).attr('y', h)
      .attr('width', x.bandwidth()).attr('height', 0)
      .attr('fill', 'url(#barGrad)').attr('rx', 4)
      .style('cursor', 'pointer')
      .on('mouseover', (event, d) => {
        tooltip.style('opacity', 1).html(`<b>${d.month}</b><br/>${d.count} sales`)
          .style('left', `${event.offsetX + 12}px`).style('top', `${event.offsetY - 28}px`)
      })
      .on('mouseout', () => tooltip.style('opacity', 0))
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      <div id="bar-tooltip" style={{
        position: 'absolute', background: CARD, border: `1px solid ${ACCENT3}`,
        borderRadius: 6, padding: '6px 10px', fontSize: 12, color: TEXT,
        pointerEvents: 'none', opacity: 0, transition: 'opacity .15s', whiteSpace: 'nowrap'
      }} />
    </div>
...
function DonutChart({ data }) {
  ...
  useEffect(() => {
    ...
    // group by a category field (status, category, type — use first found)
    const keyField = ['status', 'category', 'type', 'paymentMethod'].find(k => data[0]?.[k])
    if (!keyField) return
    ...
    g.selectAll('path').data(pie(entries)).join('path')
      ...
      .on('mouseover', function (event, d) {
        ...
        tooltip.style('opacity', 1)
          .html(`<b>${d.data.key}</b><br/>${fmt(d.data.value)}<br/>${pct}%`)
          .style('left', `${event.offsetX + 12}px`).style('top', `${event.offsetY - 36}px`)
      })
    ...
    g.append('text').attr('text-anchor', 'middle').attr('dy', '-0.2em')
      .attr('fill', TEXT).style('font-size', '13px').style('opacity', 0.6).text('Total')
    g.append('text').attr('text-anchor', 'middle').attr('dy', '1.1em')
      .attr('fill', ACCENT).style('font-size', '15px').style('font-weight', 700).text(fmt(total))
  }, [data])

  // legend
  const keyField = ['status', 'category', 'type', 'paymentMethod'].find(k => data[0]?.[k])
  const entries = keyField
    ? Array.from(d3.rollup(data, v => d3.sum(v, d => d.amount ?? d.totalAmount ?? 1), d => d[keyField] ?? 'Unknown'),
        ([k, v]) => ({ key: k, value: v })).sort((a, b) => b.value - a.value)
    : []
  const colors = [ACCENT, ACCENT2, ACCENT3, '#a78bfa', '#34d399', '#fb923c']
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 20, flexWrap: 'wrap' }}>
      <div style={{ position: 'relative' }}>
        ...
        <div id="donut-tooltip" style={{
          position: 'absolute', background: CARD, border: `1px solid ${ACCENT2}`,
          borderRadius: 6, padding: '6px 10px', fontSize: 12, color: TEXT,
          pointerEvents: 'none', opacity: 0, transition: 'opacity .15s', whiteSpace: 'nowrap'
        }} />
      </div>
...
function ScatterChart({ data }) {
  ...
  useEffect(() => {
    ...
    const points = data.map(d => ({
      date: new Date(d.createdAt ?? d.date ?? d.saleDate),
    ...
    g.selectAll('circle').data(points).join('circle')
      .attr('cx', d => x(d.date)).attr('cy', d => y(d.amount))
      .attr('r', 0).attr('fill', ACCENT2).attr('opacity', 0.75)
      .attr('stroke', 'none')
      .style('cursor', 'pointer')
      .on('mouseover', (event, d) => {
        tooltip.style('opacity', 1)
          .html(`<b>${d.date.toLocaleDateString()}</b><br/>${fmt(d.amount)}`)
          .style('left', `${event.offsetX + 12}px`).style('top', `${event.offsetY - 28}px`)
      })
      .on('mouseout', () => tooltip.style('opacity', 0))
