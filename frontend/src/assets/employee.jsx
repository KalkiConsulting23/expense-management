...
// ── Moved outside component — these are pure functions, no need to recreate ──
...
// ── Memoized Avatar: only re-renders if `name` changes ──
...
const RecurringYearTable = memo(function RecurringYearTable({
  ...
  // These totals are cheap to compute but memoizing prevents recalc on unrelated state changes
...
const ExpenseTypeGroup = memo(function ExpenseTypeGroup({
  ...
  // useMemo so years aren't recalculated on every render
...
// ── Global styles as a constant — defined ONCE outside the component ──
const GLOBAL_STYLES = `
...
// ── Inject styles once at module load — not on every render ──
...
const EmployeeTable = () => {
  ...
  useEffect(() => {
    const fetchExpenses = async () => {
      try {
        const res  = await apiFetch('https://expense-management-2-bsa7.onrender.com/api/employee/all')
    ...
    fetchExpenses()
  }, []) // apiFetch intentionally omitted — stable reference assumed

  // ── useCallback so child components don't re-render when parent state changes ──
  ...
  const handleEditCommit = useCallback(async () => {
    setEditing(prev => {
      ...
      setAllExpenses(exps => {
        const emp = exps.find(e => e._id === empId)
        if (!emp) return exps
      ...
      apiFetch(`https://expense-management-2-bsa7.onrender.com/api/employee/update-payment/${empId}`, {
        method: 'PATCH',
  ...
  const handleDeleteConfirm = useCallback(async () => {
    ...
    try {
      await apiFetch(
        `https://expense-management-2-bsa7.onrender.com/api/employee/delete/${deleteTarget._id}`,
        { method: 'DELETE' }
      )
      setAllExpenses(prev => prev.filter(e => e._id !== deleteTarget._id))
      setMonthData(prev => {
        const next = { ...prev }
  ...
  }, [deleteTarget, apiFetch])

  const handleDeleteCancel = useCallback(() => {
    if (!deleting) setDeleteTarget(null)
  }, [deleting])

  // ── Derived state memoized — only recomputes when allExpenses changes ──
