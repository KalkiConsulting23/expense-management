...
// GET all lending records (newest first) — scoped to the signed-in user
router.get('/', async (req, res) => {
  try {
    const { userId } = getAuth(req)
    const records = await Lending.find({ userId }).sort({ date: -1, createdAt: -1 })
    res.json(records)
  } catch (err) {
...
router.get('/:id', async (req, res) => {
  try {
    const { userId } = getAuth(req)
    const record = await Lending.findOne({ _id: req.params.id, userId })
...
router.delete('/:id', async (req, res) => {
  try {
    const { userId } = getAuth(req)
    const deleted = await Lending.findOneAndDelete({ _id: req.params.id, userId })
